...
func Diff(desired, live *schema.SchemaState, opt Options) (*DiffResult, error) {
	...
	changes = append(changes, diffStatistics(desired, live)...)
	changes = append(changes, diffCompositeTypes(desired, live)...)
	changes = append(changes, diffRangeTypes(desired, live)...)
	changes = append(changes, diffPublications(desired, live)...)
	changes = append(changes, diffSubscriptions(desired, live)...)
