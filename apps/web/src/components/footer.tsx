...
export function Footer() {
  return (
    <footer className="mt-24 border-t border-[--color-border] bg-[--color-background]">
      <div className="mx-auto max-w-screen-2xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8">
          {/* Brand */}
          <div className="lg:col-span-1">
            <a href="/" className="inline-flex items-center gap-2 font-semibold tracking-tight">
              <Logo className="text-[--color-primary]" size={20} />
              <span>pg-flux</span>
            </a>
            <p className="mt-3 max-w-xs text-sm leading-relaxed text-[--color-muted-foreground]">
              Declarative PostgreSQL migrations and end-to-end type generation. One model
              for your schema and your application.
            </p>
            <div className="mt-5 flex items-center gap-3 text-[--color-muted-foreground]">
              <a
          ...
          {GROUPS.map((g) => (
            <div key={g.title}>
              <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-[--color-foreground]">
                {g.title}
              </h4>
        ...
        {/* Bottom bar */}
        <div className="mt-12 flex flex-col gap-3 border-t border-[--color-border] pt-6 text-xs text-[--color-muted-foreground] sm:flex-row sm:items-center sm:justify-between">
          <p>
