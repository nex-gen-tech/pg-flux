...
### Added
- **`pg-flux update`** — self-update command that downloads and installs the latest (or a pinned `--version`) release from GitHub, verifies the SHA-256 checksum, and atomically replaces the running binary in-place. Works on macOS, Linux, and Windows.
- **`migrate rollback [N]`** — rolls back the last N applied migrations using embedded Down SQL; removes the tracking row so the migration returns to pending
- **Combined migration format** (`-- +migrate Up` / `-- +migrate Down`) — keep forward and reverse SQL in a single file; generated by `migrate generate --format combined`
- **`migrate.generate_undo` config key** — auto-generate Down SQL on every `migrate generate` without passing `--generate-undo` each time
- **`migrate.format` config key** — project-wide default format (`separate` or `combined`)
- **`migrate generate --format` flag** — per-invocation format override
- **`migrate status` DOWN column** — shows `yes`/`no` for Down SQL availability per migration
- **Exit code 6** — `migrate rollback` exits 6 when all requested migrations had no Down SQL (partial skips still exit 0)
- **Windows release binaries** — `pg-flux-windows-amd64.zip` and `pg-flux-windows-arm64.zip` now built via CGO cross-compilation in CI (mingw-w64 for amd64, llvm-mingw for arm64)

### Fixed
- **`migrate apply` ignores `_undo.sql` files** — separate-format undo files are no longer mistakenly applied as forward migrations
- **`DROP FUNCTION` undo SQL** — function argument types were incorrectly included inside the quoted identifier; fixed to generate `DROP FUNCTION IF EXISTS "schema"."name"() CASCADE` with parens outside the identifier
- **Rollback doc output** — `cli-migrate.md` and `rollback.md` examples now match actual CLI output (prefix `rollback` not `rolling back`, dry-run shows filenames only)

### Added (examples)
- **`examples/go-blog`** — minimal Go blog API with full pg-flux workflow: combined up/down migrations, rollback demo, generated Go types, and a step-by-step `JOURNEY.md`
